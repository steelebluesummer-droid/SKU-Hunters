 ...
 * props 驱动：opportunity（机会卡对象）+ existingCard（回看）+ brief（当前任务约束）
 * 企划卡通过接口生成，失败明确报错 + 重试，不再静默回退本地 mock 模板。
 */
export default function PlanCard({ opportunity, existingCard, planId = 'demo', brief }) {
  const [plan, setPlan] = useState(existingCard || null);
  ...
  const load = useCallback(async () => {
    ...
    setLogDone(false);
    try {
      const card = await generatePlanCard(opportunity.id, planId);
      setPlan(card);
    } catch (e) {
      setError(e);
      setPlan(null);
  ...
  useEffect(() => {
    ...
    setChats([]);
    load();
  }, [opportunity?.id, existingCard, load]);
  ...
  const send = async (text) => {
    ...
    try {
      ...
      setChats(cs => [...cs, { role: 'ai', text: data.reply }]);
    } catch (e) {
      setChats(cs => [...cs, { role: 'ai', text: `改稿沟通失败：${e.message}（请确认后端在线后重试）` }]);
    } finally {
      setSending(false);
    }
  };
  ...
  const gradient = GRADIENTS[plan.opportunityId] || DEFAULT_GRADIENT;
    ...
    <div>
      ...
      {plan.processLog && (
        <Card size="small" title="企划生成 · 思考过程" style={{ marginBottom: 16, background: '#f6f3ff', border: '1px solid #d9ccff' }}>
          <ProcessLog key={opportunity?.id} lines={plan.processLog} onDone={() => setLogDone(true)} />
        </Card>
      )}
      ...
      <Card>
        ...
        <h1 style={{ margin: '0 0 4px' }}>{emoji} {plan.name}</h1>
