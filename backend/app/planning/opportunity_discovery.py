...
9. 严禁在 title、summary、颜色/设计语言/风格等任何字段中写入未出现在【IP 资源库】内的知名影视、动漫、游戏、
   卡通版权角色或其名称（如蜘蛛侠、钢铁侠、蝙蝠侠、超人、美国队长、米奇、皮卡丘、哆啦A梦、奥特曼等）；
   外部联名只能使用资源库给定的 IP，资源库没有就做无外部联名的原创设计，用配色、风格、结构语言表达，
   不得借名任何具体版权角色（否则后续 AI 出图会被版权风控拦截、企划也存在侵权风险）。
...
2. title 重写为具体产品方向，并自然体现本次 IP 策略：
   ...
   注意：外部联名只能点出【IP 资源库】里给定的 IP；严禁写入资源库之外的知名影视/动漫/游戏版权角色名
   （如蜘蛛侠、钢铁侠、蝙蝠侠、米奇、皮卡丘、哆啦A梦等），资源库没有就用配色/风格/结构做原创表达。
