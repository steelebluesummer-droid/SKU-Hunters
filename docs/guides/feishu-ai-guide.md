...
**前端是深度工作室，飞书是群入口 + 传达室 + 档案室。** 群机器人归档后还会生成在线完整企划文档，
把企划正文和概念图直接交付到群里。

六步链路（约束 → 五看洞察 → 机会卡 → 创意 → 策略 → 企划卡）由后端 planning
service 统一执行；前端提供完整可视化工作室，飞书群机器人提供表单补全、机会点选、
归档和结果回传（群机器人会生成在线完整企划文档），Aily 仍提供轻量发起入口。
...
**职责：**
- 对话式收集 PlanBrief 字段，调后端 `POST /api/v1/plans/aily-create` 触发异步企划 pipeline
...
| launch_window 上新窗口 | ❌ | — |
| goals 商业目标 | ❌ | 列表 |
| mode | 🔒 | 系统字段（fixture/crawled/live），Aily 不暴露，由后端配置和严格模式控制 |
...
### 2.3 飞书群机器人 → 完整企划入口与在线文档交付

群机器人通过 WebSocket 长连接接收 @消息和卡片回调，不需要公网 webhook 或内网穿透。
它在 `backend/feishu/group_bot.py` 中调用同一套 planning pipeline：

- 一句话解析品类、联名 IP、价格和人群；缺品类/IP 或 IP 不在资源库时发送 Card 2.0 表单；
- 信息齐全后后台执行五看洞察和机会生成，返回三张方向卡；
- 用户点选方向后生成企划卡/概念图；归档后生成包含五看洞察、机会方向、完整企划案和概念图的飞书在线文档，
  再把报告卡发回原群，群内可直接查看完整产物。

### 2.4 多维表格 → 企划资产库（副本，事件驱动同步）

**定位：** 面向组织的历史企划资产视图，不是主存储。
前端主存储为后端任务状态（`pipeline.py` JSON 持久化），多维表格是副本，供检索、复用、统计。

**同步机制：** 事件驱动，不是定时任务。
- 前端或群机器人归档→ `POST /api/v1/plans/{plan_id}/actions/archive`
  → `archive_plan()` 置 `status="archived"` + `archived_at`
...
### 2.5 智能纪要 → 人机接力留痕（手动关联）

> 当前状态：智能纪要自动拉取与回写尚未接入，不是主流程能力；本节是后续落地方案，当前验收不依赖它。
...
- 后续可由后端拉取妙记列表，正则匹配 theme 关键词
- 后续匹配成功后，将纪要摘要写入对应多维表格行的"过会纪要"字段
...
1. **业务逻辑只有一套**——前端和飞书群机器人都调用后端 planning service，不在飞书侧复制一套企划算法
2. **每个产出物一个主存储地**：
   - 群机器人归档后，飞书在线云文档承载完整企划与内嵌概念图；只有改稿等深度交互回前端
   - Aily/HTTP 结果通知仍可只推机会或归档摘要 + 跳转前端，作为轻入口和兼容通知
   - 历史企划库主在多维表格，前端任务中心只列"我发起的"
3. **Aily 只做一句话能说完的事**——Aily 发起的异步任务先通过通知卡回传；群机器人闭环的完整报告在群内云文档查看，改稿仍回前端
4. **AI 入口收起来**：企业豆包/Aily 不占主布局，通过飞书侧入口或前端悬浮按钮触发

## 四、关键链路时序

### 4.1 Aily 发起企划 → pipeline 异步执行 → 飞书消息通知

```
用户（飞书Aily）  后端              飞书IM
    |              |                 |
    |--收集字段-->|                 |
    |              |--POST /plans/aily-create ->|
    |              |  (创建任务)      |
    |<--"已开始"--|                 |
    |              |                 |
    |         pipeline执行(六步链路)  |
...
### 4.2 群机器人发起 → 长连接事件 → 企划卡归档 → 在线云文档

```text
群里 @机器人
  → longconn.py 接收消息并快速返回
  → group_bot.py 后台解析/跑洞察/发机会卡
  → 用户点选机会方向
  → 生成企划卡与概念图
  → 归档、同步多维表格
...
```
用户（前端）      后端             多维表格
    |--归档-->POST /plans/{id}/actions/archive
    |           status="archived"
    |           archived_at=now
    |              |--钩子同步------>|
    |              |   新增一行     |
...
| 状态 | 模块 | 说明 |
|--------|------|------|
| ✅ | 群机器人完整企划闭环 | Card 2.0 表单、机会点选、企划卡、归档报告卡 |
| ✅ | 群内在线完整企划文档 | 归档后生成云文档，包含五看洞察、机会方向、企划案和概念图 |
| ✅ | 多维表格资产库 | 归档事件驱动同步，作为后端任务状态的副本 |
| ✅ | Aily 发起企划 | `/api/v1/plans/aily-create` 异步创建，后端主动推送结果 |
| ✅ | 前端工作室 | 洞察、机会对比、企划卡改稿和归档后的只读复盘 |
| ⏳ | source_plan_id 复用率统计 | 字段已就位，统计视图仍需完善 |
| ⏳ | 智能纪要手动关联回写 | 当前保留为后续能力，不作为主流程依赖 |
| ⏳ | 企业豆包 provider | LLM 抽象已支持替换，具体企业配置后置 |
...
| 编排实现 | Aily 拖拽编排 | 后端 planning service/pipeline（确定性六步管线） |
